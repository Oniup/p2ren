    ...
    : m_Type(info.Type)
{
    if (buffer != nullptr && buffer_size > 0)
    {
        ...
        if (!stream)
        {
            P2REN_ERROR("Failed to load embedded {} texture buffer as a stream: {}",
        ...
        if (!image_surface)
        {
            P2REN_ERROR("Failed to load embedded {} texture buffer: {}",
                        s_TextureTypeNames[(int)info.Type],
                        SDL_GetError());
            SDL_CloseIO(stream);
        }
        ...
        if (SetSizeFromSurface(image_surface, info))
            ...
            P2REN_ERROR("Failed to load embedded {} texture", s_TextureTypeNames[(int)info.Type]);
...
Texture::Texture(std::string_view path, const TextureCreateInfo info)
    : m_Type(info.Type)
{
    P2REN_ASSERT_STRING_VIEW_NULL_TERMINATED(path);

    SDL_Surface* image_surface = IMG_Load(path.data());
    if (!image_surface)
    {
        P2REN_ERROR("Failed to load surface at path {}: {}", path, SDL_GetError());
        return;
    }
...
void Texture::LoadFromSurface(SDL_Surface* image_surface, const TextureCreateInfo& info)
{
    ...
    case TextureFormat::Depth24Stencil8:
        P2REN_ERROR("Do not provide image data when setting the format to Depth24. Will still "
    ...
    if (pixel_format != image_surface->format)
    {
        ...
        if (!surface)
        {
            P2REN_ERROR("Failed to convert image texture to desired format: {}", SDL_GetError());
            return;
        }
...
bool Texture::LoadOpenGLTexture(SDL_Surface* surface, const TextureCreateInfo& info)
{
    ...
    if (info.Sample != 1)
    {
        ...
        if (info.Type != TextureType::Tex2D)
            P2REN_WARN("Texture type not set to TextureType::Tex2D when creating a multi-sampled "
    ...
    P2REN_INFO("Successfully initialized {} ({}, {})",
               s_TextureTypeNames[(int)info.Type],
               m_Width,
               m_Height);
    return true;
                                           ...
                                           int32_t base_format, int32_t output_format)
{
    if (!surface)
    {
        P2REN_ERROR("Cube map texture must provide a surface to create");
        return false;
    }
    if (info.Format > TextureFormat::RGBA)
    {
        P2REN_ERROR("Invalid texture format for creating a cube map. Must be a color format not a "
                    "depth/stencil");
        return false;
    }
...
bool Texture::SetSizeFromSurface(SDL_Surface* surface, const TextureCreateInfo& info)
{
    ...
    P2REN_ERROR("Surface is null, cannot determine texture width and height");
    return false;
}
