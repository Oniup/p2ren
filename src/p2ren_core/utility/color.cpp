...
namespace intern {
    ...
    uint8_t ConvertHexDigitToByte(std::string_view hex)
    {
        ...
        if (upper_nibble == 255 || lower_nibble)
            P2REN_FATAL("Invalid hex digit format: {}", hex);
    ...
    bool ConvertFromHexSetup(std::string_view rgb_hex, size_t& offset, size_t& source_length)
    {
        if (rgb_hex.empty())
        {
            P2REN_ERROR("Cannot convert {} into normalized color", rgb_hex);
            return false;
        }
        ...
        if (source_length >= 6)
        {
            P2REN_ERROR("Cannot convert {} into normalized color, length is greater than 6 ({})",
                        rgb_hex,
                        rgb_hex.length());
            return false;
        }
